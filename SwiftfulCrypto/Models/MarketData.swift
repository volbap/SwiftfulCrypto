...
struct MarketData: Codable {
    ...
    var marketCap: String {
        if let totalMarketCapInUSD = totalMarketCap["usd"] {
            return "$" + totalMarketCapInUSD.formattedWithAbbreviations()
        }
        return ""
    }
    
    var volume: String {
        if let volumeInUSD = totalVolume["usd"] {
            return "$" + volumeInUSD.formattedWithAbbreviations()
        }
        return ""
    }
    
    var btcDominance: String {
        if let btcMarketCapPercentage = marketCapPercentage["btc"] {
            return btcMarketCapPercentage.toPercentage()
        }
        return ""
    }
}
